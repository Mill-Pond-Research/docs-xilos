...
If your Copilot client supports a custom OpenAI-compatible endpoint, set the base URL and API key directly. This is the simplest approach — no proxy required.
  ...
  ### Configure the Copilot model
    ...
    > **Info:** The exact setting keys differ between Copilot versions. The principle is the same: set the API URL to `https://api.xilos.ai/api/v1`, the API key to your Xilos key, and the model to `xilos`.
