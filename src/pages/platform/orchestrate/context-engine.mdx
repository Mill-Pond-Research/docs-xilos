...
2. **Embed** — Each fact is embedded with the local `all-MiniLM-L6-v2` model and stored in the vector database alongside metadata (source query, model, timestamp, quality score).
...
| Setting | Default | Description |
|---------|---------|-------------|
| `embedding_model` | `all-MiniLM-L6-v2` | The local embedding model. Runs on CPU; no external API calls. |
