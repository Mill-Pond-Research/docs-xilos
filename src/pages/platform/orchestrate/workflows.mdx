...
<Callout type="info">
  Workflows complement routing rules. A routing rule decides *which* workflow to run; the workflow decides *how* multiple models cooperate to produce the answer.
</Callout>
...
<Callout type="warning">
  `best_of`, `vote`, and `summarize` invoke an additional LLM call at the Merge node. Factor this into cost estimates for high-volume workflows.
</Callout>
...
<Callout type="info">
  Workflow JSON is portable across environments. Export from a staging workspace and import into production to promote a tested workflow.
</Callout>
